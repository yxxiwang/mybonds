...
var UI = function() {
		...
		var tagDynamic = function($ipt, opts) {
				yepnope({
					test: $.fn.TagDynamic,
					load: ['/site_media/static/news/jquery.tagDynamic.js', '/site_media/static/news/jquery.tagDynamic.css', '/site_media/static/news/jquery.autocomplete.js', '/site_media/static/news/jquery.autocomplete.css'],
					complete: function() {
						$ipt.TagDynamic(opts);
					}
				});
		...
		var global_search = function() {
				...
				yepnope({
					...
					load: ['/site_media/static/news/jquery.autocomplete.js', '/site_media/static/news/jquery.autocomplete.css'],
					complete: function() {
						var $ipt = $('#search');
						/*
						$ipt.autocomplete('/search/autocomplete', {
							minChars: 1,
							matchContains: true,
							width: 274,
							scroll: false,
							...
							on_enter: function() {
								...
								window.location.href = '/search/all/' + key;
							},
							on_change: function($dom, $list) {
								...
								var key = $.trim($ipt.val());
								if (key != '') {
									$('<li class="search"><span>搜索 ' + key + '</span></li>').appendTo($list).click(function() {
										window.location.href = '/search/all/' + key
									})
								}
							},
							formatItem: function(data, i, total) {
								...
								var html = type_str == '' || type_str == '人' ? '' : '<span class="' + data.type + '">' + type_str + '</span>';
								html += type_str == '人' ? '<img src="' + data.avatar + '" class="avatar" style="float:left;margin-right:10px">' : '';
								html += data.title;
								return html;
							},
		...
		var event_history = function() {
				...
				$root.find('li').each(function() {
					...
					if ($offset.left - $m_offset.left + 620 > $('#main').width()) {
						me.find('.detail').css({
							'right': 0
						})
					} else {
					...
					var nub2 = $('#footer').height() + $('#footer').offset().top;
					if (nub1 > nub2) {
						me.find('.detail').css({
							'bottom': 0
						})
				...
				$root.find('a.plus').click(function(e) {
					e.stopPropagation();
					$root.find('.detail').hide();
					$(this).parents().filter('li').eq(0).addClass('active').css({
						opacity: 1
					}).find('.detail').fadeIn(1000).end().siblings().removeClass('active').css({
						opacity: 0.3
					})
				});
				$root.find('a.close').click(function(e) {
					e.stopPropagation();
					$(this).parents().filter('.detail').hide();
					$(this).parents().filter('li').eq(0).removeClass('active').parents().find('li').css({
						opacity: 1
					})
				})
			};
		var foot_hide = function(item, position) {
				if (position == "bottom") {
					item.css({
						"background": "url(/site_media/static/news/loading.gif) center bottom no-repeat",
						"padding-bottom": "30px"
					});
				} else {
					item.css({
...
var AjaxTooltip = function() {
		...
		var bind = function($dom, mouseover_handler) {
				if (!$container) {
					$container = $('<div id="ajaxTooltipContainer"><em class="arrow">?</em><div class="main"></div></div>').css({
						'position': 'absolute',
						'left': 0,
						'top': 0,
						'display': 'none',
...
var FormValidate = function() {
		...
		var validate = function($form, options) {
				yepnope({
					test: $.fn.validate,
					load: ['/site_media/static/news/jquery.validate.min.js'],
					complete: function() {
...
var Avatar = function() {
		var upload = function($btn) {
				yepnope({
					'test': window.qq,
					'load': ['/site_media/static/news/fileuploader.js', '/site_media/static/news/fileuploader.css'],
					'complete': function() {
						var uploader = new qq.GpkUploader({
							...
							onComplete: function(id, fileName, responseJSON) {
								if (!responseJSON || typeof responseJSON != 'object') {
									$('.avatar-upload-box .msg').html('ajax data error');
								} else if (responseJSON.error) {
									$('.avatar-upload-box .msg').html(responseJSON.error)
								} else if (responseJSON) {} else {}
							},
				...
				var uploader = new qq.GpkUploader({
					...
					onComplete: function(id, fileName, responseJSON) {
						if (!responseJSON || typeof responseJSON != 'object') {
							...
							$('.eventWeibo-upload-box .msg').html(responseJSON['message'])
						} else if (responseJSON) {
							$('.eventWeibo-upload-box .msg').html('<img width="50px" height="50px" src="' + responseJSON['info']['thumb_img'] + '" />')
						} else {}
					},
					showMessage: function(message) {
				...
				var uploader1 = new qq.GpkUploader({
					...
					onComplete: function(id, fileName, responseJSON) {
						if (!responseJSON || typeof responseJSON != 'object') {
							...
							$('.eventWeibo1-upload-box .msg').html(responseJSON['message'])
						} else if (responseJSON) {
							$('.eventWeibo1-upload-box .msg').html('<img width="50px" height="50px" src="' + responseJSON['info']['thumb_img'] + '" />')
						} else {}
					},
					showMessage: function(message) {
...
//=========Cast & Cast.wall function=========================
var Cast = {};
Cast.wall = function() {
	var dataArray = [],
		$view_mode, $filter_tags, $filter_order;
	var $thumbs_box, $list_box;
	var isloading = false,
		isending = false,
		new_data_have_more = false,
		start = -1;
	var data_server = '/news/get_cast_list/';
	var load_times = 11;
	var current_num;
	...
	var set_view_mode = function(sType) {
			$view_mode.find('li.on').removeClass('on').end().find('li.' + sType).addClass('on')
			update_view()
		};
	var set_filter_order = function(labelstr) {
			$filter_order.find("select option").removeAttr("selected").each(function() {
	...
	var get_filter_tags_str = function() {
			if (is_special_click == '推荐演讲') $('#view-order').fadeOut();
			else $('#view-order').fadeIn();
			var result = '';
			if (is_special_click) {
	...
	var set_filter_tags = function(obj) {
			if ($(obj).parents().filter("ul").eq(0).attr('id') == "special-tags") {
				$(obj).parent().addClass('specialOn').siblings().removeClass('specialOn').end().parents().find("ul#view-tags").children().removeClass('on');
			} else {
				$(obj).parent().toggleClass('on').siblings().removeClass('on').end().parent().find("#special-tags").children().removeClass('specialOn');
			}
		};
	...
	var init = function() {
			...
			$filter_tags.find('a').click(function() {
				$('html,body').animate({
					scrollTop: 0
				}, 700);
				$("#footer").css("display", "none");
				...
				if ($(this).parents().parents().filter("ul").eq(0).attr('id') == "special-tags") {
			...
			$('.cast-pin a.expand').live('click', function() {
				var $root = $(this).parents().filter('.cast-pin').eq(0);
				var $cover = $root.find('.cover');
				var default_top = $root.find('div.photo img').attr('height');
				if (!$cover.get(0).style.top || parseInt($cover.get(0).style.top) != 0) {
					$cover.animate({
						top: 0
					}, 500)
				} else {
	...
	var ajax_load = function(data, callback) {
			...
			var successHandler = function(response, textStatus, xhr) {
					...
					if (callback && typeof(callback) == 'function') callback(new_data)
				};
			UI.ajaxing();
			$.ajax(data_server + '?' + data + '&t=' + new Date().getTime()).success(successHandler).error(errorHandler)
		}
	var do_load = function(callback) {
			...
			if (start == -1) {
				if (em_mode1 == 'list') {
					if (current_num == 36) offset = 36;
					else {
						offset = 24;
						start = -1
					}
				} else {
					if (current_num == 24) start = 24;
					else start = -1;
					offset = 12;
				}
			}
			var data = get_filter_str() + '&offset=' + (start + 1) + '&limit=' + offset;
			ajax_load(data, function(x) {
				if (x.length > 0 && start < 20) {
					...
					if (new_data_have_more == true) $('.cast-list').after('<a class="cast-list-more" href="javascript:;">查看更多</a>')
					isending = true;
	...
	var render_thumbs = function(data) {
			if (data.length < 11) $('.cast-list-more').detach();
...
//============================readSeed &readSeed.wall function============
var readSeed = {};
readSeed.wall = function() {
	...
	var isloading = false,
		...
		start = -1,
		t = '';
	var data_server = '/news/load_seeds/';
	var load_times = 11;
	var current_num;
	var get_view_mode = function() {
			var tmp = $view_mode.find('li.on');
	...
	var get_filter_tags_str = function() {
			...
			if (is_special_click) {
				result += '&tag_id=' + is_special_click;
				  ...
				  var tag_name = $('#tag_name')[0].value;
				  if(tag_name !=""){
						result += '&tag_id=' + escape(tag_name);
						//result += '&tag_id=' + tag_name;
				  }
				  //alert(result);
				/*$filter_tags.find('.on').each(function() {//devwxi
	...
	var init = function() {
			$thumbs_box = $('div.cast-wall');
			$list_box = $('div.cast-list');
			//$similar_list_box = $('div.similar-list');
			$view_mode = $('nav#view-mode');
			$filter_order = $('div#view-order');
			$filter_tags = $('ul#view-tags');
			is_special_click = "";
	...
	var ajax_load = function(data, callback) {
			...
			var successHandler = function(response, textStatus, xhr) {
					...
					//alert(JSON.stringify(obj));//devwxi
					//alert("title--:"+obj.data.seeds[0].title);//devwxi
					//alert(xhr[0]);//devwxi
					//new_data_have_more = eval('(' + response + ')').data.have_more;
					new_data_have_more = obj.data.have_more;
					//alert(new_data_have_more);
					//alert(eval('(' + response + ')').data.t);
					//var new_data = eval('(' + response + ')').data.seeds
					var new_data = obj.data.seeds;
					//alert(new_data);
					if (!new_data) {
						return;
					}
					//alert(new_data);//devwxi
					dataArray = dataArray.concat(new_data);
					if (callback && typeof(callback) == 'function') callback(new_data);
				};
			UI.ajaxing();
			var otype = $('#otype')[0].value;
			if(otype=="rcm"){
				var onday = $('#onday')[0].value;
				$.ajax(data_server + '?' + data + '&tt=' + new Date().getTime() + '&t=' + otype+'&last='+onday).success(successHandler).error(errorHandler)
			}else if(otype=="beacon"){
				var similarid = $('#similarid')[0].value;
				data = 'type=similar&similarid='+similarid+'&start=' + (start + 1) + '&num=' + num;
				data_server = '/news/load_similar/';
				$.ajax(data_server + '?' + data + '&tt=' + new Date().getTime() + '&t=' + otype).success(successHandler).error(errorHandler)
			}else{
				$.ajax(data_server + '?' + data + '&tt=' + new Date().getTime() + '&t=' + otype).success(successHandler).error(errorHandler)
			}
			//$.ajax(data_server + '?' + data + '&tt=' + new Date().getTime() + '&t=' + t).success(successHandler).error(errorHandler)
	...
	var do_load = function(callback) {
			...
			/*if (t == '') {
				if (em_mode1 == 'list') {
					if (current_num == 48) num = 48;
					else num = 24;
				} else {
					start = 35;
					if (is_special_click === '') t = $('.cast-wall').attr('id');
					else t = '';
					num = 12;
				}
			}*/
			if (t == '') {
				if (em_mode1 == 'list') {
					if (current_num == 15 || start== -1 ) num = 15;//first loads news num
					else num = 15;//every time loads news num
				} else {
					start = 35;
					if (is_special_click === '') t = $('.cast-wall').attr('id');
					else t = '';
			...
			ajax_load(data, function(x) {
				temp = x.length
			  //alert(x.length+";num:"+num)//devwxi
				if(temp == 0) isending = true;
				//t = x[temp - 1]['t']
				if (x.length >= 0 && start < 80) {
					//start += x.length + 1
					start += x.length
					if (new_data_have_more == false) isending = true;
					if (x.length < num) {
						isending = true;
					}
				} else if (start >= 80) {
	...
	var render_thumbs = function(data) {
			...
			var items = [],
				...
				count = 0;
			for (var i = 0, j = data.length; i < j; i++) {
				if (data[i]['type'] == 4) count++;
			}
			for (var i = 0, j = data.length; i < j; i++) {
				if (data[i]['type'] == 4) {
					for (var z = i; z < i + count; z++) {
						bit_ad = ad_template.replace(/\{\{photo\}\}/, data[z]['thumb_img']);
						bit_ad = bit_ad.replace(/\{\{author\}\}/g, data[z]['author']);
						bit_ad = bit_ad.replace(/\{\{author_url\}\}/g, data[z]['author_url']);
						bit_ad = bit_ad.replace(/\{\{from\}\}/g, data[z]['from']);
						bit_ad = bit_ad.replace(/\{\{title\}\}/g, data[z]['title']);
						bit_ad = bit_ad.replace(/\{\{content\}\}/g, data[z]['content']);
						bit_ad = bit_ad.replace(/\{\{pubtime\}\}/g, data[z]['pub_time']);
						bit_ad = bit_ad.replace(/\{\{mark\}\}/g, 'item-type-' + z);
						bit_ad = bit_ad.replace(/\{\{width\}\}/g, 215);
						bit_ad = bit_ad.replace(/\{\{height\}\}/g, data[z]['photo_height']);
						bit_ad = bit_ad.replace(/\{\{url\}\}/g, data[z]['url']);
						bit_ad = bit_ad.replace(/\{\{type\}\}/g, data[z]['type']);
						bit_tmp += bit_ad;
					}
					bit = '<div class="type-4-thumbs item cast-pin render_thumbs" style="width:215px;">' + bit_tmp + '</div>';
					i = i + count;
				} else {
	...
	var render_list = function(data) {
			...
			//alert(data)//devwxi
			var otype = $('#otype')[0].value;
			var template ="";//<label id='{{hashid}}' class='action submit-button' style='float:right;'>相关主题</label>
			if (otype=="ppl"||otype=="rdd"){
					template = "<div class='item type-{{type}}-list cast-item' id='{{hashid}}' style='height:45px;padding-left: 15px;'>\n \
            <div class='entry {{tag_class}} cast-card'>\n \
                <div class='header' style='width: 600px;padding:0px;' id='header_links' >\n \
                    <h1><a target='_blank' style='font-size: 16px;' href='/news/research?u={{username}}&amp;likeid={{hashid}}' title='{{text}}'>{{title}}</a></h1> \n \
                </div>\n \
                <div class='meta' style='margin-top: 2px;'>\n \
                    <span style='font-size: 12px;'><a  href='/news/relatednews/?relatedid={{hashid}}' title='{{hosturl}}' >{{host}} </a> - {{elaspestr}} <i></i>  </span>\n \
                </div>\n \
      ...
      }else if (otype=="nav" ){//<i style='float:right;margin-top:-22px;'><a href='/news/beacon/?otype=beacon&similarid={{hashid}}'>相关主题</a></i>
					template = "<div class='item type-{{type}}-list cast-item' id='{{hashid}}' style='padding-left: 15px;'>\n \
            <div class='entry {{tag_class}} cast-card'>\n \
                <div class='header' style='width: 600px;padding:0px;' id='header_links' >\n \
                    <h1><a target='_blank' style='font-size: 16px;' href='/news/research?u={{username}}&amp;likeid={{hashid}}'>{{title}}</a></h1>{{similar}}\n \
                    <i style='float:right;margin-top:-22px;'><a href='#' id='titleshow' style='background: #7caee4; color:white;font-weight:bold;'>&dArr;</a></i>\n \
                </div>\n \
                <div class='meta' style='margin-top: 5px;display:none;'>\n \
                    <span style='font-size: 12px;'>{{host}} - {{elaspestr}} <i>/</i>  </span>\n \
                </div>\n \
                <div class='time' style='display: none;'>\n \
                      <span class='pubtime_year' style='float:right'>{{pubtime_year}}</span> \n \
                </div>\n \
                <div class='main' style='font-size: 13px; width: 620px;display:block;'>\n \
                      &nbsp;&nbsp;\“{{text}}\”\n \
                </div>\n \
            </div>\n \
        </div>";
      }else if(otype=="rcm" ){//padding-bottom:10px
					template = "<div class='item type-{{type}}-list cast-item' style='padding-left: 15px;'>\n \
            <div class='entry {{tag_class}} cast-card'>\n \
                <div class='meta' style='margin-top: 2px; height:2px; width: 600px; display: none;'>\n \
                    <span style='font-size: 12px;'>{{host}}<i></i>  </span>\n \
                    <span style='font-size: 12px;float:right'>{{elaspestr}} <i></i>  </span>\n \
                </div>\n \
                <div class='header' style='width: 600px;padding:0px;'  id='header_links' >\n \
                    <h1 ><a target='_blank' style='font-size: 16px;' href='/news/research?u={{username}}&amp;likeid={{hashid}}'>{{title}}</a></h1>\n \
                </div>\n \
                <div class='time' style='width: 20px; margin-left: 58px;'>\n \
                    <span class='tag_name'>{{tag}}</span><span class='pubtime_time'><span class='pubtime_year' style='font-size: 24px;'></span><span class='pubtime_date' ></span></span>\n \
                </div>\n \
                <div class='main' style='font-size: 13px; width: 620px;'>\n \
                     {{host}}&nbsp;-&nbsp;{{elaspestr}} &nbsp;&nbsp;&nbsp;\“{{text}}\”\n \
                </div>\n \
                <div style='padding-bottom:10px'></div>\n \
                <div class='bottom_line ' style='display: none;'><span></span>\n \
                </div>\n \
            </div>\n \
        </div>";
      }
      ...
      /*var relatedoc = "<table><tr><td class='font-gray-b-10pt'><a href='/research?u=wxi&amp;likeid=1472272683306320126' target='_blank'>中央财政支持新能源及可再生能源跨越式发展</a></td></tr>\n \
                     <tr><td class='font-gray-b-10pt'><a href='/research?u=wxi&amp;likeid=7479334046017418278' target='_blank'>国电电力明年将获注国电集团新疆资产</a></td></tr>   \n \
                     <tr><td class='font-gray-b-10pt'><a href='/research?u=wxi&amp;likeid=2141532167127418306' target='_blank'>中国神华联合宝新能源建电厂</a></td></tr>\n \
                    </table>";*/
			var items = [],bit;
			var tag_class = '';
			for (var i = 0; i < data.length; i++) {
			  //alert("type"+i+":"+data[i]['type']);//devwxi
				//if (data[i]['type'] == 4) continue;
				if (data[i]['tag'].toString() != '') tag_class = 'tag'
				bit = template.replace(/\{\{type\}\}/, data[i]['type']);
				bit = bit.replace(/\{\{similar\}\}/, '');
				bit = bit.replace(/\{\{similar\}\}/, "<i style='float:right;margin-top:-22px;'><a href='/news/beacon/?otype=beacon&similarid={{hashid}}'>相关主题</a></i>");
				//bit = bit.replace(/\{\{type\}\}/g, data[i]['type']);
				//bit = bit.replace(/\{\{author\}\}/g, data[i]['author']);
				//bit = bit.replace(/\{\{author_url\}\}/g, data[i]['author_url']);
				bit = bit.replace(/\{\{host\}\}/g, data[i]['host']);
				bit = bit.replace(/\{\{hosturl\}\}/g, data[i]['hosturl']);
				bit = bit.replace(/\{\{elaspestr\}\}/g, data[i]['elaspestr']);
				bit = bit.replace(/\{\{title\}\}/g, data[i]['ttl']);
				bit = bit.replace(/\{\{text\}\}/g, data[i]['tx']);
				bit = bit.replace(/\{\{hashid\}\}/g, data[i]['hashid']);
				//bit = bit.replace(/\{\{relatedCount\}\}/g, data[i]['rel_cnt']);
				//bit = bit.replace(/\{\{pubtime\}\}/g, data[i]['pub_time']);
				//bit = bit.replace(/\{\{pubtime_year\}\}/g, data[i]['pub_time'].substr(0, 4));
				//bit = bit.replace(/\{\{pubtime_date\}\}/g, data[i]['pub_time'].substr(5));
				bit = bit.replace(/\{\{width\}\}/g, 215);
				//bit = bit.replace(/\{\{height\}\}/g, data[i]['photo_height']);
				//bit = bit.replace(/\{\{url\}\}/g, data[i]['url']);
				bit = bit.replace(/\{\{username\}\}/g, data[i]['username']);
				...
				for (var xx = 0; xx < same_num.length; xx++) {
					$('.pubtime_date').each(function(i) {
						if ($(this).text() == same_num[xx]) $(this).parent().parent().parent().addClass('same same-' + xx)
					});
			...
			/*
			$('#header_links a').click(function(jQEvent) {
    			//alert('You are going to visit: ' + this.id);
    			//$(this).remove()
    			//alert('You are going to visit: ' + bit);
				if ( otype =="rdd"  ){
    		}else if( otype =="nav"){
    			if($(this).attr('id')=='titleshow'){
    				//alert($(this).attr('id'));
    				//alert($(this).parent().parent().parent().find('.main').css('display') );
    				$(this).parent().parent().parent().find('.main').hide;
            $(this).attr('id','titlehide');
    			}else if($(this).attr('id')=='titlehide'){
    				$(this).parent().parent().parent().find('.main').show();
            $(this).attr('id','titleshow');
    			} 
    		}else{
					var removediv = $(this).parent().parent().parent(); 
					removediv.parent().css('height','0px')
					$list_box.isotope('remove', removediv); 
					$list_box.isotope('insert', $('<div style="display:none"></div>'));
    	...
    	/*
    	$list_box.find('#header_links a').each(function(){
    		$(this).click(function(jQEvent) {
    			if($(this).attr('id')=='titleshow'){
    				//alert($(this).attr('id'));
    				//alert($(this).parent().parent().parent().find('.main').css('display') );
    				$(this).parent().parent().parent().find('.main').hide();
 ...
 /*
			$('#list_left').find('.isotope-item').click(function(){
				if(otype=="rdd"){
					do_similar_load(this.id,function(x) {
							render_similar_list(x);
					});
	  		}
			});
		...
		$('#list_left').find('.isotope-item').click(function(){
				$('#list_left').find('.isotope-item').each(function(){
		...
		*/
/*		//$('#list_left').find('.isotope-item').each(function(){
		//	if($(this).css('border-right-style')=="solid"){
		//			$(this).trigger("click");
		//		}
		//});
		var first =$('#list_left').find('.isotope-item')[0];
		$(first).trigger("click");
*/
    	/*
				$('#header_links label').click(function() {
				//alert($(this).html()+this.id);
					  do_similar_load(this.id,function(x) {
						render_similar_list(x);
					})
    		});*/
	...
	var do_similar_load = function(similarid,callback) {
			var num = 12;
			var data = 'type=similar&similarid='+similarid+'&start=' + (start + 1) + '&num=' + num;
			alert(data);
			ajax_load(data, function(x) {
				alert(callback)//devwxi
				if (callback && typeof(callback) == 'function') {
	...
	var render_similar_list = function(data) {
			var $similar_list_box = $('div.similar-list');
			var $similar_list_box = $list_box;
			$similar_list_box.isotope({
				itemSelector: '.cast-item',
				masonry: {
					columnWidth: 1000
			...
			//alert($similar_list_box.children())//devwxi
			$similar_list_box.isotope('remove', $similar_list_box.children());
			//alert($similar_list_box.html());
			$("#list_left").slideDown('slow');
			var otype = $('#otype')[0].value;
			var template ="";
					/*template = "<div class='item type-{{type}}-list cast-item' style='margin-top: -20px;'>\n \
            <div class='entry {{tag_class}} cast-card'>\n \
                <div class='meta' style='margin-top: 5px; height:2px; width: 300px; display: none;'>\n \
                    <span style='font-size: 8px;'>{{host}}<i></i>  </span>\n \
                    <span style='font-size: 8px;float:right'>{{elaspestr}} <i></i>  </span>\n \
                </div>\n \
                <div class='header' style='width: 300px;'  id='header_links' >\n \
                    <h1 ><a target='_blank' style='font-size: 12px;' href='/news/research?u={{username}}&amp;likeid={{hashid}}'>{{title}}</a></h1>\n \
                </div>\n \
                <div class='time' style='width: 20px; margin-left: 58px;'>\n \
                    <span class='tag_name'>{{tag}}</span><span class='pubtime_time'><span class='pubtime_year' style='font-size: 24px;'></span><span class='pubtime_date' ></span></span>\n \
                </div>\n \
                <div class='main' style='font-size: 8px; width: 320px;display:none'>\n \
                     {{host}}&nbsp;/&nbsp;{{elaspestr}} &nbsp;&nbsp;&nbsp;\“{{text}}\”\n \
                </div>\n \
                <div class='bottom_line ' style='display: none;'><span></span>\n \
                </div>\n \
					...
					template = "<div class='item type-{{type}}-list cast-item' id='{{hashid}}' style='padding-top:10px;{{leftpx}}'>\n \
            <div class='entry {{tag_class}} cast-card'>\n \
                <div class='header' style='width: 600px;' id='header_links' >\n \
                    <h1><a target='_blank' style='font-size: {{header_font_size}}' href='/news/research?u={{username}}&amp;likeid={{hashid}}'>{{title}}</a></h1> \n \
                </div>\n \
                <div class='time' style='display: none;'>\n \
                    <span class='tag_name'>{{tag}}</span><span class='pubtime_time'><span class='pubtime_year'>{{pubtime_year}}</span><span class='pubtime_date'>{{pubtime_date}}</span></span>\n \
                </div>\n \
                <div class='main' style='font-size: {{main_font_size}} width: 520px;'>\n \
                     {{host}}&nbsp;/&nbsp;{{elaspestr}} &nbsp;&nbsp;&nbsp;\“{{text}}\”\n \
                </div>\n \
            </div>\n \
        </div>";
			...
			for (var i = 0; i < data.length; i++) {
			  //alert("type"+i+":"+data[i]['type']);//devwxi
				...
				bit = template.replace(/\{\{type\}\}/, data[i]['type']);
				if(i==0){
					bit = bit.replace(/\{\{leftpx\}\}/g, "");
					bit = bit.replace(/\{\{header_font_size\}\}/g, "20px;color:#109EFF;");
					bit = bit.replace(/\{\{main_font_size\}\}/g, "12px;color:#109EFF;");
				}else{
					bit = bit.replace(/\{\{leftpx\}\}/g, "margin-left:10px;");
					bit = bit.replace(/\{\{header_font_size\}\}/g, "16px;");
					bit = bit.replace(/\{\{main_font_size\}\}/g, "12px;");
				}
				bit = bit.replace(/\{\{host\}\}/g, data[i]['host']);
				bit = bit.replace(/\{\{elaspestr\}\}/g, data[i]['elaspestr']);
				bit = bit.replace(/\{\{title\}\}/g, data[i]['ttl']);
				...
				bit = bit.replace(/\{\{tag\}\}/g, data[i]['tag']);
				bit = bit.replace(/\{\{tag_class\}\}/g, tag_class + ' tag-' + i);
				$similar_list_box.isotope('insert', $(bit));
			}
			$('#header_links a').click(function() {
				if ( otype !="rdd"){
    			//window.location.reload();
					$similar_list_box.isotope('insert', $(''));
					$similar_list_box.isotope('remove', $(this).parent().parent().parent());
					$similar_list_box.isotope('insert', $(''));
    		}
    		//$similar_list_box.isotope( 'reloadItems' );
    		//$similar_list_box.show();
     		//return confirm('You are going to visit: ' + this.href);
    	});
/*
		$('#list_left').find('.isotope-item').each(function(){
		...
		*/
		var first =$('#list_left').find('.isotope-item')[0];
		//$(first).trigger("click");
				$(first).css('border-right-color','red');
				$(first).css('border-color','#DDD');
				$(first).css('border-width','1px');
				$(first).css('border-bottom-style','solid');
				$(first).css('border-left-style','solid');

			//$list_box.hide('slow');
      //$list_box.delay(1000).css("display", "none");
    	//$similar_list_box.animate({"left": "0%",'opacity':'1'}, "slow");

    	//$('<button class="png_bg">保存灯塔</button>').insertBefore($similar_list_box);
      //$similar_list_box.delay(1000).css("float", "left");
      //$similar_list_box.delay(1000).css("width", "700px");
      //$similar_list_box.delay(1000).css("margin-left", "150px");
      //$similar_list_box.delay(1000).css("margin-right", "0px");
...
var Comment = function() {
		var $form, $list_box, id = '',
			comments_load_server = '/ajax/load_comments/',
			comment_load_server = '/ajax/load_sub_comments/',
			comment_submit_like = '/ajax/like_comment/',
			is_login_server = '/ajax/is_login/',
			generate_captcha = '/user/generate_captcha',
			check_email_server = '/users/check_email',
			get_user_header = '/user/get_user_header',
			comment_submit_server = '/ajax/comment_submit/';
		var offset = 10,
			type = 'like',
			page = 1;
		...
		var init = function(_id) {
				...
				$form = $('#comment-box');
				$list_box = $('#comments');
				$.get(is_login_server, function(response) {
					var data = eval('(' + response + ')');
					if (data.is_login) {
						$form.find('input.submit').removeClass("disable")
						$form.find('.ajax-box').hide()
						$form.find('textarea').bind('keydown click focus', function() {
							$form.find('p.anonymous,p.act').slideDown();
						});
						$list_box.find('textarea').live('keypress click', function() {
							$(this).parent().find('p.anonymous,p.act').slideDown();
						});
					} else {
						if ($('body').hasClass('login-guest')) {
							$form.find('input[name=captcha]').bind('click', function() {
								do_load_captcha($form);
								$form.find('input[name=captcha]').unbind('click')
							});
						} else if ($('body').hasClass('login-user')) {
							$form.find('p.anonymous').show();
							show_form_msg($form, '您正在使用注册帐号，请输入密码后发表评论。', 'info');
						}
						$list_box.find('textarea').live('click', function() {
							$(this).find('textarea').die('click');
							placeholder();
							$(this).parent().find('p.captcha-box,p.loginname-box,p.act').slideDown();
							if ($('body').hasClass('login-user')) {
								show_form_msg($(this).parent().parent(), '您正在使用注册帐号，请输入密码后发表评论。', 'info');
								$(this).parent().find('p.anonymous').fadeIn();
								$(this).parent().find('input[name=loginname]').val($('#comment-box').find('input[name=loginname]').val());
								$(this).parent().find('input[name=captcha]').bind('click', function() {
									do_load_captcha($(this).parent().parent().parent());
									$(this).parent().find('input[name=captcha]').unbind('click');
								});
				...
				$('.comment-form').live('submit', function(event) {
					...
					$anonymous = $('input[name="anonymous"]');
					is_anonymous = $(this).find('.anonymous-checkbox').is(':checked') ? 1 : 0;
					ajax_submit_comment(this);
					$anonymous.parent().removeClass('checked');
				});
				...
				$('.comment-form .comment-toolbar a').live('click', function(e) {
					e.preventDefault();
					var str = '#' + $(this).text() + '# ',
						pos = null,
						flag = true;
					var $f = $(this).parents().filter('form').eq(0),
						$t = $f.find('textarea').eq(0);
					$t.focus();
				...
				if ($('body').hasClass('login-user')) {} else if ($('body').hasClass('login-guest')) {}
			};
		...
		var load_captcha = function($obj) {
				...
				jQuery.get(generate_captcha + '?img_width=100&img_height=35&font_size=18&t=', {}, function(data, textStatus, xhr) {
					if (data) $obj.filter('.captcha-img').html(data);
				});
			};
		var do_load_captcha = function($obj) {
				$obj.find('p.anonymous').fadeOut();
				$obj.find('input[name=captcha][type=password]').hide();
				$obj.find('input[name=captcha][type=text]').show().focus();
		...
		var clear_form_msg = function($obj) {
				$obj.find('div.ajax-box').removeClass('success error warning sending info');
			}
		var show_form_msg = function($obj, str, type) {
				clear_form_msg($obj);
				$obj.find('div.ajax-box').html(str + '<span class="close-bt">x</span>').addClass(type).slideDown();
			}
		var refresh_option = function($obj) {
				if (!$('body').hasClass('is-login')) {
					jQuery.get(get_user_header, {}, function(data, textStatus, xhr) {
						if (data.is_login) {
							if (data.is_admin) li_class = '';
							else li_class = 'hide';
							var header_tmpl = '<li class="first"><a title="个人首页" href="/user/home/"><img class="avatar" alt="" src="' + data.avatar_url + '"></a></li>\n \
            ...
            <li class="last"><a class="loginOut png_bg" title="退出" href="/user/logout"><span>退出</span></a>\n \
							...
							$list_box.find('p.captcha-box,p.loginname-box').slideUp();
						} else {
						...
						$.get(is_login_server, function(response) {
							...
							if (data.is_login) {
								...
								$list_box.find('textarea').live('keypress click', function() {
									$list_box.find('textarea').die('keypress');
									$(this).siblings().find('input[name=captcha]').bind('focus', function() {
										do_load_captcha($(this));
									});
									$(this).parent().find('p.captcha-box,p.loginname-box,p.act').slideDown();
								});
		...
		var check_loginname = function($obj) {
				$obj.find('input[name=loginname]').live('blur', function() {
					var email = $(this),
						val = email.val(),
					...
					if (val != '邮箱' && !/^((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))$/i.test($.trim(val))) {
						...
						jQuery.post('/user/check_email', {
							...
							if (data.state == '2') do_load_password($form);
							else {
								do_load_captcha($form);
							}
						}, "json")
		...
		var ajax_submit_comment_like = function(obj) {
				var $ajax_obj = $(obj),
					user_guid = $ajax_obj.attr('rel'),
					is_like = $ajax_obj.attr('class').split(' ')[1],
					guid = $ajax_obj.attr('id'),
					parent_id = $ajax_obj.find('input[name=parent_id]').val(),
					$submit = $ajax_obj.find(':submit'),
					$content = $ajax_obj.find('#content');
				...
				$.ajax({
					...
					success: function(response, textStatus, xhr) {
						$ajax_obj.removeClass('false').addClass('true').attr('title', '已赞成').tipsy('show');
						setTimeout(function() {
							$ajax_obj.tipsy('hide')
						}, 3000);
						$ajax_obj.find('span i').text(1 + parseInt($ajax_obj.find('span i').text()));
					},
		...
		var ajax_submit_comment = function(form) {
				var $ajax_form = $(form),
					parent_id = $ajax_form.find('input[name=parent_id]').val(),
					$submit = $ajax_form.find(':submit'),
					$content = $ajax_form.find('#content'),
					email = $ajax_form.find('input[name=loginname]').val(),
					captcha = $ajax_form.find('input[name=captcha]').val(),
					password = $ajax_form.find('input[type=password]').val(),
					comment_url = window.location.href;
				$ajax_box = $ajax_form.parents().filter('.comments_list').find('.comment_list_area').eq(0)
				if ($.trim($content.val()) == '') {
					$content.focus();
				...
				$.ajax({
					...
					success: function(response, textStatus, xhr) {
						...
						try {
							...
							if ($ajax_form.attr('id') == 'comment-box') {
								if (data.error) {
									show_form_msg($ajax_form, data.message, 'warning');
									$ajax_form.find('input[name=captcha]').val('').focus();
								} else {
									show_form_msg($ajax_form, '提交成功!', 'success');
									refresh_option();
									$form.parents().filter('.side').find('#comment-box .ajax-box').eq(0).delay(1000).slideUp(function() {
										$(this).hide();
										$list_box.find('.latest').removeClass('latest');
										var html = render_comment_item(data.data);
										var $tmp = $(html).addClass('latest').animate({
								...
								if (data.error) {
									...
									show_form_msg($ajax_form, '提交成功!', 'success');
									refresh_option();
									$ajax_form.find('.ajax-box').delay(1000).slideUp(function() {
										$(this).hide();
										$ajax_box.find('.latest').removeClass('latest');
										var html = render_comment_item(data.data);
										var $tmp = $(html).addClass('latest').animate({
											backgroundColor: "#E9F3DA"
										}, 1).animate({
		...
		var reset_comment_form = function($form) {
				$form.find('input,textarea').not(':submit').not('[type="hidden"]').not('input[name=loginname]').val('');
		...
		var load_comment = function(obj, id) {
				...
				if ($obj.hasClass('close')) {
					...
					var $target = $obj.parents().filter('blockquote.parent')[0] ? $obj.parents().filter('blockquote.parent').eq(0) : $obj.parent();
					$target.nextAll().filter('blockquote.parent').remove()
				} else {
					$obj.parent().parent().parent().find('.ajax_status').addClass('loading');
					$.ajax(comment_load_server + "?guid=" + id).success(function(response) {
						$obj.parent().parent().parent().find('.ajax_status').removeClass('loading');
						var data = eval('(' + response + ')');
						if (!data || typeof data != 'object') {
							return
						}
						...
						for (var i = 0; i < data.data.length; i++) {
							var html = render_comment_item(data.data[i]);
							var $parent = $obj.parent().parent().siblings().filter('.comments_list').find('.comment_list_area');
		...
		var render_comment_item = function(data) {
				if (data.is_like) is_like_text = '已赞成';
				else is_like_text = '赞一个';
				if (data.reply_count) is_reply_text = '查看回复';
				else is_reply_text = '发表回复';
				var html = templ.replace(/\{\{user\}\}/g, data.screen_name);
				if (data.parent_id != 0) {
					html = html.replace(/\{\{reply_to\}\}/g, '<i class="reply_text" title=\'' + data.parent_screen_name + ': ' + data.parent_content + '\'> 回复 </i><a class="user" href="' + data.parent_url + '">' + data.parent_screen_name + '</a>');
				} else {
				...
				if (typeof(data.like_count) != 'undefined') {
					html = html.replace(/\{\{reply_count\}\}/g, '回复 (<i>' + data.reply_count + '</i>)');
					html = html.replace(/\{\{like_count\}\}/g, '赞 (<i>' + data.like_count + '</i>)');
				} else {
        ...
        <div class="reply-to-top"><span class="reply-to-textbox"><span class="reply-to-text">↑ 发表回复</span></span></div>';
				...
				var $page = $(pages);
				$page.find('.prev').bind('click', prev).end().find('.next').bind('click', next)
				if (page <= 1) {
		...
		var render_com_top = function() {
            ...
            <span class="sort">\n \
				...
				var $com_top = $(com_top);
				if (!$('.comment-box-type').attr('class')) $form.append($com_top);
			};
		var prev = function(e) {
				type = $form.find('.comment-box-type .sort .active').attr('id');
				var $anchor = $(e.target).hasClass('prev') ? $(e.target) : $(e.target).parent();
		...
		var next = function(e) {
				type = $form.find('.comment-box-type .sort .active').attr('id');
				var $anchor = $(e.target).hasClass('next') ? $(e.target) : $(e.target).parent();
				if ($anchor.hasClass('disabled')) {
					return
				}
				page += 1;
		...
		var reply = function(obj) {
				...
				if (has_comment > 0) {
					...
					$form = $root.parents().filter('.comment').eq(0).find('.comment-form');
					$form.find('input:submit').val('回复').end().find('textarea').focus().elastic().end().find('input[name=parent_id]').val(id).end()
					...
					$('.cancel-reply').bind('click', function() {
						$(this).parent().hide();
						$form.find('input[name=parent_id]').val($(this).parents().filter('.entry').find('.reply-bt').eq(0).attr('rel'));
					})
					return
				}
				if ($root.find('.comment-form')[0]) {} else {
                ...
                <div class="box_father">\n \
                  ...
                  <p class="act">\n \
					...
					$root.find('.comment_box_area').eq(0).append(form_temp);
					$form = $root.find('.comment-form');
					$form.find('input:submit').val('回复').end().find('textarea').focus().elastic().end().find('.comment-toolbar a').tipsy({
						gravity: 's'
					});
					window.scroll(0, $root.find('.content').offset().top - 50);
				}
				$obj.parent().parent().siblings().filter('.comments_list').slideToggle(function() {
					if ($obj.attr('original-title') == '关闭回复') {
						if (has_comment > 0) $obj.attr('title', '查看回复').tipsy("show")
						else $obj.attr('title', '发表回复').tipsy("show")
					} else $obj.attr('title', '关闭回复').tipsy("show")
					setTimeout(function() {
						$obj.tipsy("hide")
					}, 1000);
					if (has_comment == 0) {
...
var Notification = function() {
				...
				$('.wide .notice-item').hover(function() {
					...
					if ($(this).hasClass('active')) {
						$('.notice-extra').animate({
							...
							$('.side').html('');
							$('.wide .notice-item').filter('.active').removeClass('active');
						})
						return
					}
					...
					$('.notice-extra').addClass('loading').show().animate({
						left: 0
					}).load('/ajax/get_notification/' + notice_id, function() {
						var w_h = $('.wide').height(),
							s_h = $('.side').height();
						if (w_h > s_h) {
							$('.side').height(w_h)
						...
						$('.notice-extra #notice-reply').submit(Notification.reply);
						update_unread()
						oo.removeClass('unread').addClass('active').siblings().removeClass('active');
						$('.deleted-comment').parent().parent().children().not('blockquote,.content').attr('style', 'opacity:0.5');
						$('.deleted-comment').parent().children().not('.deleted-comment').attr('style', 'opacity:0.5');
					})
				})
				$('.notice-extra header a.close').live('click', function() {
					$('.notice-extra').animate({
						left: -($('.side').width())
					}, function() {
						$('.side').html('');
		...
		var reply = function(e) {
				...
				var $ajax_form = $(e.target),
					$list_box = $ajax_form.parents().find('.notice-replys').eq(0),
					parent_id = $ajax_form.find('input[name=parent_id]').val(),
					entity_id = $ajax_form.find('input[name=entity_id]').val(),
					$submit = $ajax_form.find(':submit'),
					$content = $ajax_form.find('textarea[name=content]');
				if ($.trim($content.val()) == '') {
					$content.focus();
					if ($ajax_form.find('.ajax-box')[0]) {
						clear_form_msg($ajax_form);
				...
				$.ajax({
					...
					success: function(response, textStatus, xhr) {
						...
						try {
							var data = eval('(' + response + ')');
							if (data.error) {
								show_form_msg($ajax_form, 'Error: ' + data.message, 'warning');
							} else {
								show_form_msg($ajax_form, '提交成功！', 'success');
								$ajax_form.find('.ajax-box').delay(1000).slideUp(function() {
									$(this).hide();
					...
					error: function(xhr, textStatus, errorThrown) {
						$submit.removeAttr('disabled');
						show_form_msg($ajax_form, 'Error: ' + textStatus, 'warning');
					}
				})
			};
...
var FormValidateS = function() {
		...
		var show_msg = function($obj, msg, type) {
				...
				$('<label class="msg" for="' + $obj.attr('id') + '"/>').addClass(type).html(msg).insertAfter($obj);
			};
		...
		var validate = function($form, options) {
				for (key in options.rules) {
					var $ipt = $form.find('#' + key),
						$checks = options.rules[key];
					for (key2 in $checks) {
						if ($checks[key2] && FormValidateS.check[key2]) {
							FormValidateS.check[key2]($ipt, options.message[key][key2], $checks[key2])
						}
					}
				}
				$form.submit(function(e) {
					...
					for (key in options.rules) {
						...
						for (key2 in $checks) {
							if (key2 == 'required') {
								check_required_core($ipt, options.message[key][key2], $checks[key2])
							}
							if (key2 == 'equalTo') {
								check_equalTo_core($ipt, options.message[key][key2], $checks[key2])
							}
						}
					}
...
var NewsLoad = function() {
		...
		$(window).scroll(function() {
			...
			var limit = 10;
			if (w_h + $(document).scrollTop() + limit >= d_h) {
				if (range_type_check == range_type) {} else if (is_more) {
					do_load();
		...
		$(".news #view-tags a").live("click", function() {
			$("#footer").css("display", "none");
			$(".main_content").empty().append("<div id='observe' class='clearfix'></div>");
			var curTag = $(this).text();
			var preTag = $(this).parent().siblings().find("span");
			preTag.parent().removeClass("on").html("<a href='javascript:void(0)' rel=" + preTag.attr('rel') + ">" + preTag.text() + "</a>")
			$(this).parent().addClass("on");
			$(this).parent().html("<span rel=" + $(this).attr('rel') + ">" + curTag + "</span>");
			...
			if (view_mode == "thumbs") {
				...
				var tag_name = $("#view-tags li[class='on']").find("span").text();
				var tag_id1 = $("#view-tags li[class='on']").find("span").attr("rel");
				if (tag_id1) {
					$(".main_content").empty().append("<div id='data-list' class='clearfix'><dl><dt><span class='tag'>" + tag_name + "</span></dt><dd></dd></dl></div>");
					ListNewsLoad("", "center", "/ajax/load_seeds_list/", tag_id1);
				} else {
					$(".main_content").empty().append("<div id='data-list' class='clearfix'></div>");
		...
		$(".news #view-mode").find("a").click(function() {
...
var ListNewsLoad = function(start_time1, position, url, tag_id1) {
		...
		$(window).scroll(function() {
			...
			var limit = 50;
			if (w_h + $(document).scrollTop() + limit >= d_h) {
				if (start_time_check == start_time) {} else {
					var tag_name = $("#view-tags li[class='on']").find("span").text();
					var tag_id1 = $("#view-tags li[class='on']").find("span").attr("rel");
					if (tag_id1) {
						doLoad(start_time, "bottom", "/ajax/load_seeds_list/", tag_id1);
					} else {
						doLoad(start_time, "bottom", "/ajax/get_one_day_seed/");
					}
...
jQuery(document).ready(function($) {
	...
	$(".geek_observe p a[href*='http'],.geek_observe .nest a[href*='http']").attr("target", "_blank");
});
*/
jQuery(document).ready(function($) {
	$('.footer-content .weixin').mouseenter(function() {
	...
	Feedback.init();
	//readSeed.wall.init();
	if ($('body').hasClass('feedback')) Feedback.feedback_page();
	$(".geek_observe p a[href*='http'],.geek_observe .nest a[href*='http']").attr("target", "_blank");
});

//------------maybe rocket_up function===============devwxi
jQuery(function() { 
	jQuery('a[href*=#]').live('click', function(e) {
		...
		if (location.pathname.replace(/^\//, '') == this.pathname.replace(/^\//, '') && location.hostname == this.hostname) {
			...
			if (!$url) {
				...
				if ($url.indexOf('section') >= 0) targetOffset = $(document).scrollTop() - 50;
				return false;
			}
		}
	});
...
var Tags = function() {
		...
		var guid = $.trim($("#tags_guid").val()),
			user_guid = $.trim($("#user_guid").val()),
			relation_type = $.trim($("#relation_type").val());
		...
		var tipMsg = function(msg) {
				...
				$error_div.find("strong").text(msg).end().css('display', 'inline-block');
				setTimeout(function() {
					$error_div.fadeOut()
				}, 3000);
				$("#tagsAddTxt").focus();
		...
		var showModify = function() {
				var $remove_div = $(this).parents('.tags'),
					$tags_add_div = $(this).next('.tags_add');
		...
		var cancelModify = function() {
				var tagsAdd_div = $(this).parent(".tags_add"),
					remove_div = $(this).parents('.tags'),
					val = $.trim($("#tagsAddTxt").val());
				tagsAdd_div.hide();
		...
		var confirmModify = function(sign) {
				...
				var data = "tag_name=" + val + "&guid=" + guid + "&user_guid=" + user_guid + "&type=" + relation_type;
				$.post(Url.addTag, data, function(result) {
					switch (result.error) {
					case 0:
						$("#tagsAddTxt").val("").focus();
						$ul.append("<li><span><a target='_blank' href='/tag/" + val + "'>" + val + "</a></span><a href='#' class='del'></a></li>");
						break;
					case 1:
						tipMsg("已存在此标签请重新添加");
		...
		var tags_search = function($ipt) {
				yepnope({
					...
					load: ['/site_media/static/news/jquery.autocomplete.js', '/site_media/static/news/jquery.autocomplete.css'],
					complete: function() {
						$ipt.autocomplete(Url.searchTag, {
							tips: '',
							minChars: 1,
							cacheLength: 0,
							mustMatch: false,
							...
							formatItem: function(data, i, total) {
								if (i === total) {
									$(".tags_s_list").parents('.ac_results').addClass('tags_ac_results');
								}
								return "<span class='tags_s_list'>" + data.label + "</span>";
							},
							formatMatch: function(data, i, total) {
								return data.label;
							},
		...
		var load_tags = function(obj) {
				var $obj = $(obj),
					total = ($("#tags-total").text()) * 1,
					tag_name = $.trim($(".tags-page h1 strong").text()),
					$ul = $(".tags-page .tags-page-list ul");
				var li_html = function(guid, entity_guid, title, name, time, highlight, abstract_cont, subtype_id) {
						if (subtype_id == 8) {
							return '<li>' + '     <h3><a href="/read/view/' + entity_guid + '">' + title + '</a></h3>' + '     <p class="info"><a href="/user/home/index/' + guid + '">' + name + "</a> / " + time + '</p>' + '     <div class="cont">' + abstract_cont + '</div>' + ' </li>';
						} else if (subtype_id == 5) {
							return '<li>' + '     <h3 class="video"><a href="/cast/view/' + entity_guid + '">' + title + '</a><em></em></h3>' + '     <p class="info">' + time + '</p>' + '     <div class="cont">' + highlight + '</div>' + ' </li>';
						}
					}
						...
						$.post(Url.loadTag, {
							...
							if (data.toString() !== "NULL") {
								...
								$.each(data, function(i, item) {
									str += li_html(item.guid, item.entity_guid, item.title, item.screen_name, item.time_updated, item.highlight, item["abstract"], item.subtype_id);
								})
								$ul.append(str);
...
Tags.modify('.tags .tags_modify', '#tagsAddTxt', '#tagsAddBtn', '#tagsAddCancel');
